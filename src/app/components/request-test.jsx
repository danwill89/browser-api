...
export default function RequestTest() {
    ...
    const handleContinue = () => {
        if (!licenseData || !vrcData) {
            alert(
                "Please upload both your license and vehicle registration before continuing."
            );
            return;
        }

        // Extract the required fields from the uploaded documents
        const givenName = Array.from(licenseData)
            .find(([key]) => key === "org.iso.18013.5.1")?.[1]
            ?.get("given_name");
        const familyName = Array.from(licenseData)
            .find(([key]) => key === "org.iso.18013.5.1")?.[1]
            ?.get("family_name");
        const ageOver21 = Array.from(licenseData)
            .find(([key]) => key === "org.iso.18013.5.1")?.[1]
            ?.get("age_over_21");
        const registrationNumber = Array.from(vrcData)
            .find(([key]) => key === "org.iso.7367.1")?.[1]
            ?.get("registration_number");
        const dateOfRegistration = Array.from(vrcData)
            .find(([key]) => key === "org.iso.7367.1")?.[1]
            ?.get("date_of_registration");
        const vehicleIdentificationNumber = Array.from(vrcData)
            .find(([key]) => key === "org.iso.7367.1")?.[1]
            ?.get("vehicle_identification_number");
        ...
        router.push(
            `/permit-application/confirmation?${queryParams.toString()}`
        );
    };
    ...
    const onRequest = async () => {
        ...
        try {
            ...
            const data = await decodeCredential(credentialResponse.data.vp_token['mDL-request-demo']);
            setLicenseData(data);
        } catch (err) {
            alert(err);
        }
    ...
    const requestVRC = async () => {
        ...
        try {
            ...
            const data = await decodeCredential(
                credentialResponse.data.vp_token['vrc-request-demo'],
                ["org.iso.7367.1"]
            );
            setVrcData(data);
        } catch (err) {
                ...
                {!licenseData ? (
                    <div className="space-y-4">
                        <p className="text-gray-600">
                            Please upload your driver's license to proceed with
